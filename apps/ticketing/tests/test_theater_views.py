...
class TheaterAPIsTestCase(APITestCase):
    ...
    def test_customers_cannot_create_theaters(self):
        ...
        response = self.client.post(self.theaters_url, sample_data, headers=headers, format='json')
    ...
    def test_theater_can_be_updated(self):
        ...
        data = {"name": "Juja Comrades Cinema",
            "location": {"lat": -1.2921, "lng": 36.8219},
            "location_description": "Next to JKUAT",
            "town": "Juja Town",
            "number_of_seats": 100,
        ...
        self.assertEqual(Theater.objects.get(id=theater.id).name, "Juja Comrades Cinema")
    ...
    def test_theater_can_be_patched(self):
        ...
        data = {"name": "Juja Comrades Cinema and Conference", "location_description": "Next to JKUAT Gate C"}
        update_response = self.client.patch(patch_url, data=data, headers=headers, format='json')
        self.assertEqual(update_response.status_code, status.HTTP_200_OK)
        self.assertEqual(Theater.objects.get(id=theater.id).name, "Juja Comrades Cinema and Conference")
        self.assertEqual(Theater.objects.get(id=theater.id).location_description, "Next to JKUAT Gate C")
