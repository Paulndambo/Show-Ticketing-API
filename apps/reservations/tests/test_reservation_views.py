...
class ReservationAPITestCase(APITestCase):
    def setUp(self) -> None:
        ...
        self.dummy_reservations = Reservation.objects.bulk_create(
            ...
            status="Active",
            notification_send=False), Reservation(user=self.user,
            show=self.show,
            seat=self.seat,
            ticket_cost=self.show.ticket_cost,
            reservation_type="Single Ticket",
    ...
    def test_seat_can_be_reserved_when_authenticated(self):
        ...
        response = self.client.post(self.reserve_seat_url, data=data, headers=headers, format='json')
    ...
    def test_reservation_can_be_fetched(self):
        ...
        response = self.client.get(f"{self.reservations_url}{reservation.id}/", headers=headers)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_reservation_can_be_updated(self):
        ...
        response = self.client.patch(f"{self.reservations_url}{reservation.id}/", data={"status": "Cancelled"}, headers=headers, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        
        fetch_response = self.client.get(f"{self.reservations_url}{reservation.id}/", headers=headers)
    ...
    def test_reservation_can_be_deleted(self):
        ...
        response = self.client.delete(f"{self.reservations_url}{reservation.id}/", headers=headers)
